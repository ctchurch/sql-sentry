// The majority of this application is taken from safesql by Stripe.  Safesql detects
//   non-const SQL strings indicating dynamic SQL.  We take this a step further and
//   map constant SQL strings into a whitelist regular expression that we can later use
//   to validate SQL at runtime
...
func main() {
	...
	flag.StringVar(&whitelistfile, "o", "", "Whitelist file")
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: %s [-q] [-o whitelist] [-v] package1 [package2 ...]\n", os.Args[0])
		flag.PrintDefaults()
	}
	...
	validsql, bad := FindValidSQL(res.CallGraph, qms)
	if len(bad) == 0 {
		if !quiet {
			fmt.Println(`You're safe from SQL injection! Yay \o/`)
			if whitelistfile != "" {
				err = WriteWhitelist(validsql, whitelistfile)
				if err != nil {
					fmt.Printf("error writing whitelist file: %v\n", err)
				}
			}
		}
	...
	fmt.Println("instead of building queries from strings.  Until you fix this, we cannot produce")
	fmt.Println("a valid whitelist of SQL")
	os.Exit(1)
}
...
// FindValidSQL returns the valid SQL patterns in a call graph as well as the "bad" methods that can produce dynamic SQL
func FindValidSQL(cg *callgraph.Graph, qms []*QueryMethod) ([]string, []ssa.CallInstruction) {
	...
	for _, m := range qms {
		...
		for _, edge := range node.In {
			...
			v := args[m.Param]
			if param, ok := v.(*ssa.Const); !ok {
				bad = append(bad, edge.Site)
			} else {
				validsql = append(validsql, fmt.Sprintf("%s", param.Value))
			}
		}
	}
...
func WriteWhitelist(validsql []string, filename string) error {
	for i, sql := range validsql {
		validsql[i] = regexp.QuoteMeta(sql)
	}

	re, err := syntax.Parse(strings.Join(validsql, "|"), syntax.POSIX)
